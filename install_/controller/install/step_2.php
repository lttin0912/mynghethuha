...
class ControllerInstallStep2 extends Controller {
	...
	public function index() {
		$this->language->load('install_/step_2');
		
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->response->redirect($this->url->link('install_/step_3'));
		}
		...
		$data['action'] = $this->url->link('install_/step_2');
		...
		$data['back'] = $this->url->link('install_/step_1');

		$data['footer'] = $this->load->controller('common/footer');
		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');

		$this->response->setOutput($this->load->view('install_/step_2', $data));
	}
